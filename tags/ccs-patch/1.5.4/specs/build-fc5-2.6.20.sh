...
+%define release %(R="$Revision: 1.2320 $"; RR="${R##: }"; echo ${RR%%?})%{?dist}.fc5_tomoyo_1.5.4
...
+# TOMOYO Linux
+tar -zxf $RPM_SOURCE_DIR/ccs-patch-1.5.4-20080510.tar.gz
...
+  sed -i -e "s/^CONFIG_DEBUG_INFO=.*/# CONFIG_DEBUG_INFO is not set/" -- .config
   ...
   echo "# $Arch" > configs/$i
EOF
mv kernel-2.6.spec kernel-2.6.20-1.2320.fc5_tomoyo_1.5.4.spec || die "Can't rename spec file."
echo ""
echo ""
echo ""
echo "Edit /tmp/kernel-2.6.20-1.2320.fc5_tomoyo_1.5.4.spec if needed, and run"
echo "rpmbuild -bb /tmp/kernel-2.6.20-1.2320.fc5_tomoyo_1.5.4.spec"
