...
static int AddMountACL(const char *dev_name, const char *dir_name, const char *fs_type, const unsigned int enable, const unsigned int disable)
{
	...
	if (!IsCorrectPath(dev_name, 0, 0, 0, __FUNCTION__) || (dev = SaveName(dev_name)) == NULL) goto out;
	if (!IsCorrectPath(dir_name, 1, 0, 1, __FUNCTION__) || (dir = SaveName(dir_name)) == NULL) goto out;
