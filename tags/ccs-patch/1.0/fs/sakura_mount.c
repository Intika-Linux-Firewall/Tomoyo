...
static int AddMountACL(const char *dev_name, const char *dir_name, const char *fs_type, const unsigned int enable, const unsigned int disable)
{
	...
	if (!ptr) {
		...
		memset(new_entry, 0, sizeof(MOUNTS_ENTRY));
		new_entry->next = NULL;
	...
	}
	{
		...
		const unsigned int enabled = ptr->enabled_options, disabled = ptr->disabled_options;
		if (strcmp(fs, MOUNT_REMOUNT_KEYWORD) == 0) {
			printk("Allow remount %s with options %s%s%s%s%s.\n", dir,
				   enabled == 0 && disabled == 0 ? "defaults" : "",
			...
			printk("Allow mount %s %s %s\n", fs, dev, dir);
		} else if ((type = get_fs_type(fs)) != NULL) {
			if (type->fs_flags & FS_REQUIRES_DEV) {
				printk("Allow mount -t %s %s %s with options %s%s%s%s%s.\n", fs, dev, dir,
					   enabled == 0 && disabled == 0 ? "defaults" : "",
					   enabled & MS_RDONLY ? "ro "     : (disabled & MS_RDONLY ? "rw "   : ""),
					   enabled & MS_NOSUID ? "nosuid " : (disabled & MS_NOSUID ? "suid " : ""),
					   enabled & MS_NODEV  ? "nodev "  : (disabled & MS_NODEV  ? "dev "  : ""),
				...
				printk("Allow mount %s on %s with options %s%s%s%s%s.\n", fs, dir,
					   enabled == 0 && disabled == 0 ? "defaults" : "",
					   enabled & MS_RDONLY ? "ro "     : (disabled & MS_RDONLY ? "rw "   : ""),
					   enabled & MS_NOSUID ? "nosuid " : (disabled & MS_NOSUID ? "suid " : ""),
					   enabled & MS_NODEV  ? "nodev "  : (disabled & MS_NODEV  ? "dev "  : ""),
...
int CheckMountPermission(char *dev_name, char *dir_name, char *type, unsigned long *flags)
{
	...
	case 0:
		break;
	default:
		printk("SAKURA-ERROR: More than one of 'remount' 'move' 'bind' are given.\n");
		return -EINVAL;
	}
	if (*flags & MS_REMOUNT) {
		*flags &= ~MS_REMOUNT;
	...
	} else if (dir_name && *dir_name && type && *type) {
		MOUNTS_ENTRY *ptr;
	...
	cleanup:
		if (requested_dev_name) kfree(requested_dev_name);
		if (requested_dir_name) kfree(requested_dir_name);
	...
	if (error_flag) {
		...
		if (type && strcmp(type, MOUNT_REMOUNT_KEYWORD) == 0) {
		...
		} else if (type && (strcmp(type, MOUNT_BIND_KEYWORD) == 0 || strcmp(type, MOUNT_MOVE_KEYWORD) == 0)) {
		...
		if (realname2) kfree(realname2);
		if (realname1) kfree(realname1);
		if (is_enforce) return -EPERM;
	}
	return 0;
}
...
int AddMountPolicy(char *data)
{
	...
	fs = cp2;
	return AddMountACL(dev, dir, fs, enable, disable);
 out:
	return -EINVAL;
}
...
int DelMountPolicy(char *data)
{
	...
	cp2 = data; if ((cp = strchr(cp2, ' ')) == NULL) goto out; *cp = '\0'; dev = cp2;
	cp2 = cp + 1; if ((cp = strchr(cp2, ' ')) == NULL) goto out; *cp = '\0'; dir = cp2;
	cp2 = cp + 1; if ((cp = strchr(cp2, ' ')) != NULL) *cp = '\0'; fs = cp2;
	for (ptr = mount_list.next; ptr; ptr = ptr->next) {
		if (strcmp(ptr->dev_name, dev) == 0 && strcmp(ptr->dir_name, dir) == 0 && strcmp(ptr->fs_type, fs) == 0) {
			ptr->is_deleted = 1;
			break;
...
int ReadMountPolicy(IO_BUFFER *head)
{
	...
	while (ptr) {
		...
		if (ptr->is_deleted == 0 && io_printf(head, KEYWORD_ALLOW_MOUNT "%s %s %s %s%s%s%s\n", ptr->dev_name, ptr->dir_name, ptr->fs_type,
											  enabled & MS_RDONLY ? "ro "     : (disabled & MS_RDONLY ? "rw "   : ""),
											  enabled & MS_NOSUID ? "nosuid " : (disabled & MS_NOSUID ? "suid " : ""),
											  enabled & MS_NODEV  ? "nodev "  : (disabled & MS_NODEV  ? "dev "  : ""),
											  enabled & MS_NOEXEC ? "noexec " : (disabled & MS_NOEXEC ? "exec " : ""))) break;
		ptr = ptr->next;
	}
	return ptr ? -ENOMEM : 0;
}
