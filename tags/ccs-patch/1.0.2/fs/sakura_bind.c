...
static int AddReservedEntry(const unsigned short int min_port, const unsigned short int max_port)
{
	...
	if (min_port != max_port) printk("%sSAKURA-NOTICE: Port %u-%u reserved.\n", ccs_log_level, min_port, max_port);
	else printk("%sSAKURA-NOTICE: Port %u reserved.\n", ccs_log_level, min_port);
	return 0;
}
