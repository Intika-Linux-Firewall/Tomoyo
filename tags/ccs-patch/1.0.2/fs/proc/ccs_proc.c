...
static void CreateEntry(const char *name, const mode_t mode, struct proc_dir_entry *parent, const int key)
{
...
void CCSProc_Init(void)
{
	...
	CreateEntry("delete_domain",    0200, policy_dir, CCS_POLICY_DELETEDOMAIN);
	CreateEntry("update_domain",    0200, policy_dir, CCS_POLICY_UPDATEDOMAIN);
