...
if [ ! -r kernel-2.6.18-274.5.AXS3.src.rpm ]
then
    wget http://ftp.miraclelinux.com/pub/Asianux/Server/3.0/updates/src/kernel-2.6.18-274.5.AXS3.src.rpm || die "Can't download source package."
fi
rpm --checksig kernel-2.6.18-274.5.AXS3.src.rpm || die "Can't verify signature."
rpm -ivh kernel-2.6.18-274.5.AXS3.src.rpm || die "Can't install source package."

cd /usr/src/asianux/SOURCES/ || die "Can't chdir to /usr/src/asianux/SOURCES/ ."
if [ ! -r ccs-patch-1.7.3-20120301.tar.gz ]
then
    wget -O ccs-patch-1.7.3-20120301.tar.gz 'http://sourceforge.jp/frs/redir.php?f=/tomoyo/43375/ccs-patch-1.7.3-20120301.tar.gz' || die "Can't download patch."
fi
...
+%define release %{revision}.5%{dist}_tomoyo_1.7.3p2
...
+# TOMOYO Linux
+tar -zxf %_sourcedir/ccs-patch-1.7.3-20120301.tar.gz
