...
if [ ! -r kernel-source-2.6.34.7-0.2.2.src.rpm ]
then
    wget http://download.opensuse.org/update/11.3/rpm/src/kernel-source-2.6.34.7-0.2.2.src.rpm || die "Can't download source package."
fi
rpm --checksig kernel-source-2.6.34.7-0.2.2.src.rpm || die "Can't verify signature."
rpm -ivh kernel-source-2.6.34.7-0.2.2.src.rpm || die "Can't install source package."

if [ ! -r kernel-default-2.6.34.7-0.2.2.nosrc.rpm ]
then
    wget http://download.opensuse.org/update/11.3/rpm/src/kernel-default-2.6.34.7-0.2.2.nosrc.rpm || die "Can't download source package."
fi
rpm --checksig kernel-default-2.6.34.7-0.2.2.nosrc.rpm || die "Can't verify signature."
rpm -ivh kernel-default-2.6.34.7-0.2.2.nosrc.rpm || die "Can't install source package."
...
if [ ! -r ccs-patch-1.7.2-20100918.diff ]
then
    wget -O ccs-patch-1.7.2-20100918.diff 'http://svn.sourceforge.jp/cgi-bin/viewcvs.cgi/*checkout*/trunk/1.7.x/ccs-patch/patches/ccs-patch-2.6.34-suse-11.3.diff?revision=3994&root=tomoyo' || die "Can't download patch."
fi
...
+patch -sp1 < %_sourcedir/ccs-patch-1.7.2-20100918.diff
