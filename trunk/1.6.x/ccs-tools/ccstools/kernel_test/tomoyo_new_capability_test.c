...
static int write_policy(void) {
	...
	while (fgets(buffer, sizeof(buffer) - 1, fp)) {
		...
		if (domain_found) {
			if (!strncmp(buffer, "allow_capability ", 17) && !strcmp(buffer + 17, capability)) {
				policy_found = 1;
				break;
			}
		}
...
static void SetCapability(void) {
	...
	memset(buffer, 0, sizeof(buffer));
	snprintf(buffer, sizeof(buffer) - 1, "MAC_FOR_CAPABILITY::%s=enforcing\n", capability);
	WriteStatus(buffer);
}
...
static void UnsetCapability(void) {
	static char buffer[1024];
	memset(buffer, 0, sizeof(buffer));
	snprintf(buffer, sizeof(buffer) - 1, "MAC_FOR_CAPABILITY::%s=disabled\n", capability);
	WriteStatus(buffer);
}
...
static void StageCapabilityTest(void) {
	...
	
	{
		...
		SetCapability();
		if (write_policy()) {
			show_result(connect(fd3, (struct sockaddr *) &addr, sizeof(addr)), 1);
			delete_policy();
			show_result(connect(fd4, (struct sockaddr *) &addr, sizeof(addr)), 0);
		}
		UnsetCapability();
	...
	SetCapability();
	if (write_policy()) {
		if (!is_kernel26) show_result((int) create_module("", 0), 1);
		show_result(init_module("", NULL), 1);
		show_result(delete_module(""), 1);
		delete_policy();
		if (!is_kernel26) show_result((int) create_module("", 0), 0);
	...
	if (write_policy()) {
		...
		show_result(bind(fd1, (struct sockaddr *) &addr, sizeof(addr)), 1);
		unlink(tmp1);
		delete_policy();
		show_result(bind(fd2, (struct sockaddr *) &addr, sizeof(addr)), 0);
		unlink(tmp1);
		if (fd1 != EOF) close(fd1);
		if (fd2 != EOF) close(fd2);
	}
	...
	if (write_policy()) {
		show_result(reboot(LINUX_REBOOT_MAGIC1, LINUX_REBOOT_MAGIC2,
				   0x0000C0DE /* Use invalid value so that the system won't reboot. */, NULL), 1);
		delete_policy();
		show_result(reboot(LINUX_REBOOT_MAGIC1, LINUX_REBOOT_MAGIC2,
				   0x0000C0DE /* Use invalid value so that the system won't reboot. */, NULL), 0);
	}
	UnsetCapability();
...
#ifdef __NR_tgkill
		if (is_kernel26) show_result(tgkill(pid, gettid(), SIGINT), 1);
#endif
		delete_policy();
		show_result(kill(pid, SIGINT), 0);
		show_result(tkill(gettid(), SIGINT), 0);
#ifdef __NR_tgkill
		if (is_kernel26) show_result(tgkill(pid, gettid(), SIGINT), 0);
#endif
	}
	UnsetCapability();
	...
	if (write_policy()) {
		...
		if (is_kernel26) show_result(sys_kexec_load(0, 0, NULL, 0), 1);
#endif
		delete_policy();
#ifdef __NR_sys_kexec_load
		if (is_kernel26) show_result(sys_kexec_load(0, 0, NULL, 0), 0);
#endif
	}
	UnsetCapability();
	...
	if (write_policy()) {
		...
		show_result(setpriority(PRIO_PROCESS, pid, getpriority(PRIO_PROCESS, pid)), 1);
		delete_policy();
		show_result(nice(0), 0);
		show_result(setpriority(PRIO_PROCESS, pid, getpriority(PRIO_PROCESS, pid)), 0);
	}
	UnsetCapability();
	...
	if (write_policy()) {
		...
		show_result(ioctl(fd, 0 /* Use invalid value so that nothing happen. */), 1);
		delete_policy();
		show_result(ioctl(fd, 0 /* Use invalid value so that nothing happen. */), 0);
		close(fd);
	}
	UnsetCapability();
