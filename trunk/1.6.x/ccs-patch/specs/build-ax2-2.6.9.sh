...
if [ ! -r kernel-2.6.9-78.12AXS2.src.rpm ]
then
    wget http://ftp.miraclelinux.com/pub/Miracle/ia32/standard/4.0/updates/SRPMS/kernel-2.6.9-78.12AXS2.src.rpm || die "Can't download source package."
fi
rpm -ivh kernel-2.6.9-78.12AXS2.src.rpm || die "Can't install source package."
...
--- kernel-2.6.spec	2009-05-22 17:05:55.000000000 +0900
+++ kernel-2.6.spec	2009-06-05 16:09:51.000000000 +0900
@@ -34,7 +34,7 @@
 #
 %define axbsys %([ "%{?WITH_LKST}" -eq 0 ] && echo || echo .lkst)
 %define dist AXS2
-%define release 78.12%{?dist}%{axbsys}
+%define release 78.12%{?dist}%{axbsys}_tomoyo_1.6.8
 %define sublevel 9
