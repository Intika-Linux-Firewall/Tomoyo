...
then
    wget http://download.opensuse.org/update/11.0/rpm/src/kernel-source-2.6.25.5-1.1.src.rpm || die "Can't download source package."
fi
...
if [ ! -r ccs-patch-2.6.25.5-1.1_SUSE.diff ]
then
    wget -O ccs-patch-2.6.25.5-1.1_SUSE.diff 'http://svn.sourceforge.jp/cgi-bin/viewcvs.cgi/*checkout*/trunk/1.6.x/ccs-patch/patches/ccs-patch-2.6.25.5-1.1_SUSE.diff?root=tomoyo' || die "Can't download patch."
fi
...
+patch -sp1 < %_sourcedir/ccs-patch-2.6.25.5-1.1_SUSE.diff
...
EOF
mv kernel-default.spec kernel-2.6.25.5-1.1-default_tomoyo_1.6.1.spec || die "Can't rename spec file."
echo ""
echo ""
echo ""
echo "Edit /tmp/kernel-2.6.25.5-1.1-default_tomoyo_1.6.1.spec if needed, and run"
echo "rpmbuild -bb /tmp/kernel-2.6.25.5-1.1-default_tomoyo_1.6.1.spec"
