...
if [ ! -r kernel-2.6.32-754.11.1.el6.src.rpm ]
then
    wget http://vault.centos.org/6.10/updates/Source/SPackages/kernel-2.6.32-754.11.1.el6.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-2.6.32-754.11.1.el6.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-2.6.32-754.11.1.el6.src.rpm || die "Can't install source package."

cd ~/rpmbuild/SOURCES/ || die "Can't chdir to ~/rpmbuild/SOURCES/ ."
if [ ! -r ccs-patch-1.8.5-20190305.tar.gz ]
then
    wget -O ccs-patch-1.8.5-20190305.tar.gz 'https://osdn.jp/frs/redir.php?f=/tomoyo/49684/ccs-patch-1.8.5-20190305.tar.gz' || die "Can't download patch."
fi
...
+# TOMOYO Linux
+tar -zxf %_sourcedir/ccs-patch-1.8.5-20190305.tar.gz
