...
if [ ! -r kernel-2.6.32-573.26.1.el6.src.rpm ]
then
    wget http://vault.centos.org/6.7/updates/Source/SPackages/kernel-2.6.32-573.26.1.el6.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-2.6.32-573.26.1.el6.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-2.6.32-573.26.1.el6.src.rpm || die "Can't install source package."

cd /root/rpmbuild/SOURCES/ || die "Can't chdir to /root/rpmbuild/SOURCES/ ."
if [ ! -r ccs-patch-1.8.5-20160505.tar.gz ]
then
    wget -O ccs-patch-1.8.5-20160505.tar.gz 'http://osdn.jp/frs/redir.php?f=/tomoyo/49684/ccs-patch-1.8.5-20160505.tar.gz' || die "Can't download patch."
fi
...
+%define kernel_prereq  fileutils, module-init-tools, initscripts >= 8.11.1-1, grubby >= 7.0.4-1
 %if %{with_dracut}
 %define initrd_prereq  dracut-kernel >= 004-388.el6
 %else
@@ -473,7 +473,7 @@
 AutoProv: yes\
 %{nil}
...
+# TOMOYO Linux
+tar -zxf %_sourcedir/ccs-patch-1.8.5-20160505.tar.gz
