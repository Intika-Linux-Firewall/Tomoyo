...
if [ ! -r kernel-2.6.32-504.30.3.el6.src.rpm ]
then
    wget http://vault.centos.org/6.6/updates/Source/SPackages/kernel-2.6.32-504.30.3.el6.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-2.6.32-504.30.3.el6.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-2.6.32-504.30.3.el6.src.rpm || die "Can't install source package."
...
+%define kernel_prereq  fileutils, module-init-tools, initscripts >= 8.11.1-1, grubby >= 7.0.4-1
 %if %{with_dracut}
 %define initrd_prereq  dracut-kernel >= 004-356.el6_6.3
 %else
@@ -473,7 +473,7 @@
 AutoProv: yes\
 %{nil}
