...
if [ ! -r kernel-3.10.0-693.11.6.el7.src.rpm ]
then
    wget http://vault.centos.org/centos/7/updates/Source/SPackages/kernel-3.10.0-693.11.6.el7.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-3.10.0-693.11.6.el7.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-3.10.0-693.11.6.el7.src.rpm || die "Can't install source package."

cd ~/rpmbuild/SOURCES/ || die "Can't chdir to ~/rpmbuild/SOURCES/ ."
if [ ! -r ccs-patch-1.8.5-20171111.tar.gz ]
then
    wget -O ccs-patch-1.8.5-20171111.tar.gz 'http://osdn.jp/frs/redir.php?f=/tomoyo/49684/ccs-patch-1.8.5-20171111.tar.gz' || die "Can't download patch."
fi
...
+# TOMOYO Linux
+tar -zxf %_sourcedir/ccs-patch-1.8.5-20171111.tar.gz
