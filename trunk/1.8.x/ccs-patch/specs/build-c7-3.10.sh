...
if [ ! -r kernel-3.10.0-862.3.2.el7.src.rpm ]
then
    wget http://vault.centos.org/centos/7/updates/Source/SPackages/kernel-3.10.0-862.3.2.el7.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-3.10.0-862.3.2.el7.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-3.10.0-862.3.2.el7.src.rpm || die "Can't install source package."
