...
_Bool check_remote_host(void)
{
	...
	if (!fp ||
	    fscanf(fp, "%u.%u.%u", &tomoyo_major, &tomoyo_minor, &tomoyo_rev) < 2 ||
	    tomoyo_major != 1 || tomoyo_minor != 7) {
...
int main(int argc, char *argv[])
{
	...
	else if (!strcmp(argv0, "tomoyo-selectpolicy"))
	...
	else if (!strcmp(argv0, "tomoyo-queryd"))
		ret = queryd_main(argc, argv);
	else if (!strcmp(argv0, "tomoyo-auditd"))
...
show_version:
	 ...
	 * "hard links".
	 */
	printf("tomoyotools version 1.7.0+ build 2009/09/14\n");
