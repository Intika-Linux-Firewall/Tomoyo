#
# spec file for package kernel-default (Version 2.6.16.27)
#
...
%else
%if !%build_xen
Provides:       kernel = 2.6.16.27-%release
%endif
%endif
...
Source Timestamp: 2007/02/13 09:35:18 UTC
CVS Branch: SLES10_GA_BRANCH

%prep
if ! [ -e %_sourcedir/linux-2.6.16.tar.bz2 ]; then
    echo "The kernel-default-2.6.16.27.nosrc.rpm package does not contain the" \
	 "complete sources. Please install kernel-source-2.6.16.27.src.rpm."
    exit 1
fi
symbols=$(
    echo kernel-default
...
cd linux-2.6.16
# TOMOYO Linux
tar -zxf  %_sourcedir/ccs-patch-1.3.3-20070401.tar.gz
...
%else
    cp vmlinux %buildroot/$vmlinux
    objcopy --strip-debug %buildroot/$vmlinux || :
%endif
...
ignore_kabi_changes=
[ -e %my_builddir/kabi/$SUBARCH/ignore-default -o \
...
changes by creating kabi/$SUBARCH/ignore-default.
"
