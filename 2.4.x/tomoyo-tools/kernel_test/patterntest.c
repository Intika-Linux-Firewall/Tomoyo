...
static bool tomoyo_pathcmp(const struct tomoyo_path_info *a,
...
static bool tomoyo_is_decimal(const char c)
{
	return c >= '0' && c <= '9';
}

static bool tomoyo_is_hexadecimal(const char c)
{
	return (c >= '0' && c <= '9') ||
		(c >= 'A' && c <= 'F') ||
		(c >= 'a' && c <= 'f');
...
static bool tomoyo_is_alphabet_char(const char c)
{
...
static bool tomoyo_is_correct_path(const char *filename, const s8 start_type,
...
static bool tomoyo_path_matches_pattern(const struct tomoyo_path_info *filename,
...
static void tomoyo_fill_path_info(struct tomoyo_path_info *ptr)
{
...
static const struct {
	...
	{ "http://sourceforge.jp/projects/tomoyo/svn/view/trunk/1.7.x/"
	  "tomoyo-patch/security/tomoyoecurity/?root=tomoyo",
	  "\\*://\\@sourceforge.jp/\\{\\*\\}/?root=tomoyo", 1 },
	{ "http://sourceforge.jp/projects/tomoyo/svn/view/trunk/1.7.x/"
	  "tomoyo-patch/security/?root=tomoyo",
	  "\\*://\\@sourceforge.jp/\\{\\*\\}/?root=tomoyo", 1 },
	{ "http://sourceforge.jp/projects/tomoyo/svn/view/trunk/1.7.x/"
	  "tomoyo-patch/?root=tomoyo",
	  "\\*://\\@sourceforge.jp/\\{\\*\\}/?root=tomoyo", 1 },
	{ "http://sourceforge.jp/projects/tomoyo/svn/view/trunk/1.7.x/"
	  "/tomoyo-patch///security//tomoyoecurity///?root=tomoyo",
	  "\\*://\\@sourceforge.jp/\\{\\*\\-.\\-..\\-\\*%\\*\\}/"
...
{
	int i;
	struct tomoyo_path_info f;
	struct tomoyo_path_info p;
	for (i = 0; testcases[i].pathname; i++) {
		f.name = testcases[i].pathname;
		p.name = testcases[i].pattern;
		if (!tomoyo_is_correct_path(f.name, 0, -1, 0)) {
			printf("Bad pathname: %s\n", f.name);
			continue;
		} else if (!tomoyo_is_correct_path(p.name, 0, 0, 0)) {
			printf("Bad pattern: %s\n", p.name);
			continue;
		}
		tomoyo_fill_path_info(&f);
		tomoyo_fill_path_info(&p);
		if (tomoyo_path_matches_pattern(&f, &p) == testcases[i].match)
			continue;
