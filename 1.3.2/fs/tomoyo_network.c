...
static int AddNetworkEntry(const int is_ipv6, const int operation, const u32 *min_address, const u32 *max_address, const u16 min_port, const u16 max_port, struct domain_info *domain, const int is_delete, const struct condition_list *condition)
{
	struct acl_info *ptr;
	int error = -ENOMEM;
	const u8 type = is_ipv6 ? TYPE_IPv6_NETWORK_ACL : TYPE_IPv4_NETWORK_ACL;
	const u16 hash = operation;
	...
	if (!is_delete) {
		...
		while (1) {
			if (ptr->type == type && ptr->u.w == hash && ptr->cond == condition) {
				if (is_ipv6) {
		...
		for (ptr = domain->first_acl_ptr; ptr; ptr = ptr->next) {
			if (ptr->type != type || ptr->is_deleted || ptr->u.w != hash || ptr->cond != condition) continue;
			if (is_ipv6) {
...
static int CheckNetworkEntry(const int is_ipv6, const int operation, const u32 *address, const u16 port)
{
	...
	for (ptr = domain->first_acl_ptr; ptr; ptr = ptr->next) {
		if (ptr->type != type || ptr->is_deleted || ptr->u.w != hash || CheckCondition(ptr->cond, NULL)) continue; 
		if (is_ipv6) {
			if (((IPv6_NETWORK_ACL_RECORD *) ptr)->min_port <= port && port <= ((IPv6_NETWORK_ACL_RECORD *) ptr)->max_port && memcmp(((IPv6_NETWORK_ACL_RECORD *) ptr)->min_address, address, 16) <= 0 && memcmp(address, ((IPv6_NETWORK_ACL_RECORD *) ptr)->max_address, 16) <= 0) break;
		} else {
			if (((IPv4_NETWORK_ACL_RECORD *) ptr)->min_port <= port && port <= ((IPv4_NETWORK_ACL_RECORD *) ptr)->max_port && ((IPv4_NETWORK_ACL_RECORD *) ptr)->min_address <= ip && ip <= ((IPv4_NETWORK_ACL_RECORD *) ptr)->max_address) break;
		}
	}
	if (ptr) {
