...
if [ ! -r kernel-2.6.18-194.8.AXS3.src.rpm ]
then
    wget http://ftp.miraclelinux.com/pub/Asianux/Server/3.0/updates/src/kernel-2.6.18-194.8.AXS3.src.rpm || die "Can't download source package."
fi
rpm --checksig kernel-2.6.18-194.8.AXS3.src.rpm || die "Can't verify signature."
...
if [ ! -r ccs-patch-1.7.2-20100929.diff ]
then
    wget -O ccs-patch-1.7.2-20100929.diff 'http://svn.sourceforge.jp/cgi-bin/viewcvs.cgi/*checkout*/trunk/1.7.x/ccs-patch/patches/ccs-patch-2.6.18-asianux-3.diff?revision=4021&root=tomoyo' || die "Can't download patch."
fi
...
+%define release 194.8%{?dist}_tomoyo_1.7.2p3
...
+patch -sp1 < %_sourcedir/ccs-patch-1.7.2-20100929.diff
+
