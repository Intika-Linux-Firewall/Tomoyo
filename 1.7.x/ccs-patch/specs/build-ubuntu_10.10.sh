...
VERSION=`apt-cache search ^linux-image-2.6.35-..- | cut -b 13-21 | awk ' { print $1 }' | sort -r | uniq | head -n 1`
...
if [ ! -r ccs-patch-1.7.2-20100917.diff ]
then
    wget -O ccs-patch-1.7.2-20100917.diff 'http://svn.sourceforge.jp/cgi-bin/viewcvs.cgi/*checkout*/trunk/1.7.x/ccs-patch/patches/ccs-patch-2.6.35-ubuntu-10.10.diff?revision=3991&root=tomoyo' || die "Can't download patch."
fi
...
tar -zxf /root/rpmbuild/SOURCES/ccs-patch-1.7.2-20100804.tar.gz || die "Can't extract patch."
#patch -p1 < patches/ccs-patch-2.6.35-ubuntu-10.10.diff || die "Can't apply patch."
patch -p1 < /root/rpmbuild/SOURCES/ccs-patch-1.7.2-20100917.diff || die "Can't apply patch."
...
for i in debian.master/abi/2.6.35-*/*/ ; do touch $i/ccs.ignore; done
