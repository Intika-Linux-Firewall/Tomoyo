...
static int AddBindEntry(const int is_stream, const unsigned short int min_port, const unsigned short int max_port, struct domain_info *domain, const int is_delete)
{
	struct acl_info *ptr;
	int error = -ENOMEM;
	const unsigned int type_hash = MAKE_ACL_TYPE(TYPE_BIND_ACL) + MAKE_ACL_HASH(is_stream ? 1 : 0);
...
int CheckBindEntry(const int is_stream, const unsigned short int port)
{
	...
	if (is_enforce) return CheckSupervisor("%s\n" KEYWORD_ALLOW_BIND "%s/%u\n", domain->domainname, is_stream ? "TCP" : "UDP", port);
	if (CheckCCSAccept(CCS_TOMOYO_MAC_FOR_BINDPORT)) AddBindEntry(is_stream, port, port, domain, 0);
	return 0;
}

int AddBindPolicy(char *data, struct domain_info *domain, const int is_delete)
{
	unsigned int from, to;
	if (sscanf(data, "TCP/%u-%u", &from, &to) == 2) {
		if (from <= to && to < 65536) return AddBindEntry(1, from, to, domain, is_delete);
	} else if (sscanf(data, "TCP/%u", &from) == 1) {
		if (from < 65536) return AddBindEntry(1, from, from, domain, is_delete);
	} else if (sscanf(data, "UDP/%u-%u", &from, &to) == 2) {
		if (from <= to && to < 65536) return AddBindEntry(0, from, to, domain, is_delete);
	} else if (sscanf(data, "UDP/%u", &from) == 1) {
		if (from < 65536) return AddBindEntry(0, from, from, domain, is_delete);
	}
