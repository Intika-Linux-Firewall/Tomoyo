...
static int AddConnectEntry(const int is_stream, const unsigned short int min_port, const unsigned short int max_port, struct domain_info *domain, const int is_delete)
{
	struct acl_info *ptr;
	int error = -ENOMEM;
...
int CheckConnectEntry(const int is_stream, const unsigned short int port)
{
	...
	if (CheckCCSAccept(CCS_TOMOYO_MAC_FOR_CONNECTPORT)) AddConnectEntry(is_stream, port, port, domain, 0);
	return 0;
}

int AddConnectPolicy(char *data, struct domain_info *domain, const int is_delete)
{
	unsigned int from, to;
	if (sscanf(data, "TCP/%u-%u", &from, &to) == 2) {
		if (from <= to && to < 65536) return AddConnectEntry(1, from, to, domain, is_delete);
	} else if (sscanf(data, "TCP/%u", &from) == 1) {
		if (from < 65536) return AddConnectEntry(1, from, from, domain, is_delete);
	} else if (sscanf(data, "UDP/%u-%u", &from, &to) == 2) {
		if (from <= to && to < 65536) return AddConnectEntry(0, from, to, domain, is_delete);
	} else if (sscanf(data, "UDP/%u", &from) == 1) {
		if (from < 65536) return AddConnectEntry(0, from, from, domain, is_delete);
	}
