...
static bool is_policy_manager(void)
{
	...
	if (task->tomoyo_flags & CCS_TASK_IS_POLICY_MANAGER)
		return true;
	if (!manage_by_non_root && (current_uid() || current_euid()))
		return false;
	list1_for_each_entry(ptr, &policy_manager_list, list) {
...
#endif
#ifdef CONFIG_SAKURA
	printk(KERN_INFO "SAKURA: 1.6.6   2009/02/02\n");
#endif
#ifdef CONFIG_TOMOYO
	printk(KERN_INFO "TOMOYO: 1.6.6   2009/02/02\n");
#endif
