...
static const char *which(const char *program)
{
	...
	char *cp = getenv("PATH");
	if (!cp)
		cp = "/sbin:/bin:/usr/sbin:/usr/bin:/usr/local/bin";
...
static void scan_dir_for_pattern2(const char *dir, const unsigned int flags)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (type == DT_REG)
			...
			scan_dir_for_pattern2(path, flags);
...
static void scan_dir_for_pattern(const char *dir)
{
	...
	return scan_dir_for_pattern2(path, SCANDIR_MUST_CONTAIN_NUMBER_WILDCARD);
...
static void scan_dir_for_read(const char *dir)
{
	...
	memset(path, 0, sizeof(path));
	strncpy(path, dir, sizeof(path) - 1);
	return scan_dir_for_pattern2(dir, SCANDIR_MAY_CONTAIN_NUMBER_WILDCARD);
	
}

static void scan_dir_for_depth2(const char *dir, int depth, int *max_depth)
{
	struct dirent **namelist;
	int n = scandir(dir, &namelist, scandir_dir_filter, 0);
	int len;
	int i;
	if (n < 0)
		return;
	...
	for (i = 0; i < n; i++) {
		...
		if (type == DT_UNKNOWN)
			type = revalidate_path(path);
		if (type == DT_DIR)
			scan_dir_for_depth2(path, depth + 1, max_depth);
		free((void *) namelist[i]);
	}
	free((void *) namelist);
}
...
static int scan_depth(const char *dir)
{
	...
	memset(path, 0, sizeof(path));
	strncpy(path, dir, sizeof(path) - 1);
	scan_dir_for_depth2(dir, 0, &max_depth);
	return max_depth;
}
...
static void scan_dir_for_read2(const char *dir, _Bool flag, int const_len)
{
	struct dirent **namelist;
	int n = scandir(dir, &namelist, scandir_file_and_dir_filter, 0);
	int len;
	int i;
	_Bool done = 0;
	if (n < 0)
	...
	for (i = 0; i < n; i++) {
		...
		if (type == DT_REG && flag && !done) {
			...
			const _Bool f = flag ||
				...
				strstr(name, "fonts") ||
				strstr(name, "icons");
			scan_dir_for_read2(path, f, f != flag ?
...
static void make_file_patterns(void)
{
	struct stat buf;
	FILE *fp;
	const char *cp;
	static const char *man_dirs[] = {
		"/usr/share/man", "/usr/X11R6/man"
	};
	static const char *spool_dirs[] = {
	...
	static const char *initializer_programs[] = {
		"/sbin/cardmgr", "/sbin/getty", "/sbin/init", "/sbin/klogd", "/sbin/mingetty", "/sbin/portmap", "/sbin/rpc.statd",
		"/sbin/syslogd", "/sbin/udevd", "/usr/X11R6/bin/xfs", "/usr/bin/dbus-daemon-1", "/usr/bin/jserver", "/usr/bin/mDNSResponder",
		"/usr/bin/nifd", "/usr/bin/spamd", "/usr/sbin/acpid", "/usr/sbin/afpd", "/usr/sbin/anacron", "/usr/sbin/apache2",
		"/usr/sbin/apmd", "/usr/sbin/atalkd", "/usr/sbin/atd", "/usr/sbin/cannaserver", "/usr/sbin/cpuspeed", "/usr/sbin/cron",
		"/usr/sbin/crond", "/usr/sbin/cupsd", "/usr/sbin/dhcpd", "/usr/sbin/exim4", "/usr/sbin/gpm", "/usr/sbin/hald",
		"/usr/sbin/htt", "/usr/sbin/httpd", "/usr/sbin/inetd", "/usr/sbin/logrotate", "/usr/sbin/lpd", "/usr/sbin/nmbd",
		"/usr/sbin/papd", "/usr/sbin/rpc.idmapd", "/usr/sbin/rpc.mountd", "/usr/sbin/rpc.rquotad", "/usr/sbin/sendmail.sendmail",
		"/usr/sbin/smartd", "/usr/sbin/smbd", "/usr/sbin/squid", "/usr/sbin/sshd", "/usr/sbin/vmware-guestd", "/usr/sbin/vsftpd",
		"/usr/sbin/xinetd"
	};
	int i;
	...
	for (i = 0; i < elementof(man_dirs); i++) {
		const int max_depth = scan_depth(man_dirs[i]);
		int j;
		for (j = 0; j < max_depth; j++) {
			int k;
			printf("file_pattern %s", man_dirs[i]);
			for (k = 0; k <= j; k++)
				printf("/\\*");
			puts("/\\*");
		}
	...
	keyword = NULL;
	for (i = 0; i < elementof(spool_dirs); i++) {
		if (lstat(spool_dirs[i], &buf) || !S_ISDIR(buf.st_mode))
			continue;
		printf("file_pattern ");
		printf_encoded(spool_dirs[i], 0);
		puts("/\\*");
	}
	...
	keyword = "allow_read";
	for (i = 0; i < elementof(common_files); i++) {
		char *cp = realpath(common_files[i], NULL);
		if (!cp)
			continue;
		printf_encoded(cp, 0);
		free(cp);
	...
	keyword = "initialize_domain";
	for (i = 0; i < elementof(initializer_programs); i++) {
		char *cp = realpath(initializer_programs[i], NULL);
		if (!cp)
			continue;
		if (!access(cp, X_OK))
			printf_encoded(cp, 0);
	...
	if (!lstat("/root/.emacs.d", &buf) && S_ISDIR(buf.st_mode))
		puts("file_pattern /root/.emacs.d/auto-save-list/.saves-\\$-\\*");
	...
	cp = which("spamd");
 	if (cp) {
		if (fgrep("/tmp/spamassassin-$$", cp)) {
			puts("file_pattern /tmp/spamassassin-\\$/");
			puts("file_pattern /tmp/spamassassin-\\$/.spamassassin/");
			puts("file_pattern /tmp/spamassassin-\\$/.spamassassin/auto-whitelist\\*");
		}
		if (fgrep("spamd-$$-init", cp)) {
	...
	cp = which("mail");
	if (cp) {
		if (fgrep("/mail.XXXXXX", cp))
			puts("file_pattern /tmp/mail.\\?\\?\\?\\?\\?\\?");
		if (fgrep("/mail.RsXXXXXXXXXX", cp))
			puts("file_pattern /tmp/mail.RsXXXX\\?\\?\\?\\?\\?\\?");
		if (fgrep("/mail.ReXXXXXXXXXX", cp))
			puts("file_pattern /tmp/mail.ReXXXX\\?\\?\\?\\?\\?\\?");
		if (fgrep("/mail.XXXXXXXXXX", cp))
			puts("file_pattern /tmp/mail.XXXX\\?\\?\\?\\?\\?\\?");
		if (fgrep("/mail.RxXXXXXXXXXX", cp))
			puts("file_pattern /tmp/mail.RxXXXX\\?\\?\\?\\?\\?\\?");
		if (fgrep("/mail.RmXXXXXXXXXX", cp))
			puts("file_pattern /tmp/mail.RmXXXX\\?\\?\\?\\?\\?\\?");
		if (fgrep("/mail.RqXXXXXXXXXX", cp))
	...
	if (!access("/etc/init.d/autofs", X_OK) && fgrep("/tmp/autofs.XXXXXX", "/etc/init.d/autofs"))
	...
	if (!lstat("/var/lib/mlocate", &buf) && S_ISDIR(buf.st_mode))
		puts("file_pattern /var/lib/mlocate/mlocate.db.\\?\\?\\?\\?\\?\\?");
	...
	cp = which("logwatch");
	if (cp) {
		if (fgrep("/var/cache/logwatch", cp)) {
			puts("file_pattern /var/cache/logwatch/logwatch.XX\\?\\?\\?\\?\\?\\?/");
			puts("file_pattern /var/cache/logwatch/logwatch.XX\\?\\?\\?\\?\\?\\?/\\*");
		} else {
	...
	cp = which("logrotate");
	if (cp && fgrep("/logrotate.XXXXXX", cp)) {
		puts("file_pattern /tmp/logrotate.\\?\\?\\?\\?\\?\\?");
		puts("aggregator /tmp/logrotate.\\?\\?\\?\\?\\?\\? /tmp/logrotate.tmp");
	}
	
	/* Make patterns for cardmgr(8). */
	cp = which("cardmgr");
	if (cp && fgrep("%s/cm-%d-%d", cp))
	...
	if (!access("/usr/lib/cron/run-crons", X_OK) && fgrep("/tmp/run-crons.XXXXXX", "/usr/lib/cron/run-crons")) {
		puts("file_pattern /tmp/run-crons.\\?\\?\\?\\?\\?\\?/");
		puts("file_pattern /tmp/run-crons.\\?\\?\\?\\?\\?\\?/run-crons.\\*");
	}
	...
	if (!lstat("/var/lib/pgsql", &buf) && S_ISDIR(buf.st_mode)) {
		...
		puts("file_pattern /var/lib/pgsql/data/base/\\$/\\$");
		puts("file_pattern /var/lib/pgsql/data/base/global/pg_database.\\$");
		puts("file_pattern /var/lib/pgsql/data/base/\\$/pg_internal.init.\\$");
		puts("file_pattern /var/lib/pgsql/data/base/\\$/pg_internal.init");
		puts("file_pattern /var/lib/pgsql/data/base/pgsql_tmp/pgsql_tmp\\*");
		...
		puts("file_pattern /var/lib/pgsql/data/pg_multixact/members/\\X");
		puts("file_pattern /var/lib/pgsql/data/pg_multixact/offsets/\\X");
	...
	if (!lstat("/var/lib/postgres", &buf) && S_ISDIR(buf.st_mode)) {
		...
		puts("file_pattern /var/lib/postgres/data/global/pgstat.tmp.\\$");
	...
	if (!lstat("/var/lib/postgresql", &buf) && S_ISDIR(buf.st_mode)) {
		...
		puts("file_pattern /var/lib/postgresql/\\*/main/base/\\$/\\$");
		puts("file_pattern /var/lib/postgresql/\\*/main/base/\\$/pg_internal.init.\\$");
		puts("file_pattern /var/lib/postgresql/\\*/main/base/\\$/PG_VERSION");
		puts("file_pattern /var/lib/postgresql/\\*/main/global/\\$");
		puts("file_pattern /var/lib/postgresql/\\*/main/global/\\$/pg_auth.\\$");
		puts("file_pattern /var/lib/postgresql/\\*/main/global/\\$/pg_database.\\$");
		puts("file_pattern /var/lib/postgresql/\\*/main/pg_clog/\\X");
		puts("file_pattern /var/lib/postgresql/\\*/main/pg_multixact/members/\\X");
		puts("file_pattern /var/lib/postgresql/\\*/main/pg_multixact/offsets/\\X");
		puts("file_pattern /var/lib/postgresql/\\*/main/pg_subtrans/\\X");
		puts("file_pattern /var/lib/postgresql/\\*/main/pg_xlog/\\X");
		puts("file_pattern /var/lib/postgresql/\\*/main/pg_xlog/xlogtemp.\\$");
	}
	...
	if (fgrep("Fedora Core", "/etc/issue") || fgrep("CentOS", "/etc/issue")) {
	...
	if (!lstat("/var/lib/init.d", &buf) && S_ISDIR(buf.st_mode)) {
		...
		puts("file_pattern /var/lib/init.d/depcache.\\?\\?\\?\\?\\?\\?\\?");
		puts("file_pattern /var/lib/init.d/treecache.\\?\\?\\?\\?\\?\\?\\?");
	}
