...
if [[ "x${CCS_SCRIPT_DIR}" = "x" ]]; then
	printf '%s\n' 'Variable $CCS_SCRIPT_DIR not defined.'
	exit 1
fi

if [[ ! -f "${CCS_SCRIPT_DIR}/global_functions" ]]; then
	error "${CCS_SCRIPT_DIR}/global_functions: no such file"
fi

. "${CCS_SCRIPT_DIR}/global_functions"
...
download_file "${URL_SERVICE_FILES}/ccs-auditd.service" "ccs-auditd.service"
download_file "${URL_SERVICE_FILES}/ccs-notifyd.service" "ccs-notifyd.service"
