 ...
 *
 *  /dev needs to be writable, but this means that files on /dev might be tampered.
 *  SYAORAN can restrict combinations of (filename, filetype) that the system can create.
 *  The filetype is one of directory, regular file, FIFO, unix domain socket,
 *  symbolic link, character device file, block device file.
 *
...
static int Syaoran_Initialize(struct super_block *sb, void *data)
{
	...
	if (first) {
		first = 0;
		printk("SYAORAN: 1.0.2 2006/02/09\n");
	}
	{
