...
static void stage_file_test(void)
{
	...
	policy = "allow_read /bin/true "
		"if 0=0 0=0 10=10-100";
	write_domain_policy(policy, 0);
	show_result(uselib("/bin/true"), 1);
	write_domain_policy(policy, 1);
	show_result(uselib("/bin/true"), 0);

	policy = "allow_execute /bin/true if task.uid!=10 0=0";
	write_domain_policy(policy, 0);
	fflush(stdout);
	fflush(stderr);
	pipe(pipe_fd);
	...
	policy = "allow_read /dev/null if 0!=12345678 1=1 "
		"1!=3";
	write_domain_policy(policy, 0);
	...
	policy = "allow_read /dev/null if 0!=0666";
	write_domain_policy(policy, 0);
	fd = open("/dev/null", O_RDONLY);
	show_result(fd, 1);
	if (fd != EOF)
	...
	policy = "allow_read /dev/null if 0!=0777";
	write_domain_policy(policy, 0);
	fd = open("/dev/null", O_RDONLY);
	show_result(fd, 1);
	if (fd != EOF)
	...
	policy = "allow_read /dev/null if "
		"0!=01234 0!=012345 0!=0123";
	write_domain_policy(policy, 0);
	fd = open("/dev/null", O_RDONLY);
	show_result(fd, 1);
	if (fd != EOF)
	...
	snprintf(pbuffer, sizeof(pbuffer) - 1,
		 "allow_write %s if 0x11!=%u 0x11!=%u",
	...
	policy = "allow_read/write /tmp/fifo if 0!=1234";
	mkfifo2("/tmp/fifo");
	write_domain_policy(policy, 0);
	...
	policy = "allow_read /dev/null if 0=0";
	write_domain_policy(policy, 0);
	fd = open("/dev/null", O_RDONLY);
	show_result(fd, 1);
	if (fd != EOF)
	...
	policy = "allow_write /dev/null if 0=0";
	write_domain_policy(policy, 0);
	...
	policy = "allow_read/write /dev/null if task.uid=0";
	write_domain_policy(policy, 0);
	...
	policy = "allow_create /tmp/open_test 0644 "
		"if 0=task.uid";
	write_domain_policy(policy, 0);
	policy = "allow_write /tmp/open_test if 0=0";
	write_domain_policy(policy, 0);
	...
	policy = "allow_write /tmp/open_test if task.uid=0 0=0";
	write_domain_policy(policy, 0);
	...
	policy = "allow_truncate /tmp/truncate_test if task.uid=0";
	write_domain_policy(policy, 0);
