...
if [ ! -r kernel-source-2.6.31.14-0.8.1.src.rpm ]
then
    wget http://download.opensuse.org/update/11.2/rpm/src/kernel-source-2.6.31.14-0.8.1.src.rpm || die "Can't download source package."
fi
rpm --checksig kernel-source-2.6.31.14-0.8.1.src.rpm || die "Can't verify signature."
rpm -ivh kernel-source-2.6.31.14-0.8.1.src.rpm || die "Can't install source package."

if [ ! -r kernel-default-2.6.31.14-0.8.1.nosrc.rpm ]
then
    wget http://download.opensuse.org/update/11.2/rpm/src/kernel-default-2.6.31.14-0.8.1.nosrc.rpm || die "Can't download source package."
fi
rpm --checksig kernel-default-2.6.31.14-0.8.1.nosrc.rpm || die "Can't verify signature."
rpm -ivh kernel-default-2.6.31.14-0.8.1.nosrc.rpm || die "Can't install source package."

cd /usr/src/packages/SOURCES/ || die "Can't chdir to /usr/src/packages/SOURCES/ ."
if [ ! -r ccs-patch-1.8.2-20110707.tar.gz ]
then
    wget -O ccs-patch-1.8.2-20110707.tar.gz 'http://sourceforge.jp/frs/redir.php?f=/tomoyo/49684/ccs-patch-1.8.2-20110707.tar.gz' || die "Can't download patch."
fi
...
+# TOMOYO Linux
+tar -zxf %_sourcedir/ccs-patch-1.8.2-20110707.tar.gz
