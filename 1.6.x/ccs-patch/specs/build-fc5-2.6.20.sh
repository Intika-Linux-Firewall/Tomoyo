...
+%define release %(R="$Revision: 1.2320 $"; RR="${R##: }"; echo ${RR%%?})%{?dist}.fc5_tomoyo_1.6.0
...
+# TOMOYO Linux
+tar -zxf $RPM_SOURCE_DIR/ccs-patch-1.6.0-20080401.tar.gz
...
+  sed -i -e "s/^CONFIG_DEBUG_INFO=.*/# CONFIG_DEBUG_INFO is not set/" -- .config
   ...
   echo "# $Arch" > configs/$i
EOF
mv kernel-2.6.spec kernel-2.6.20-1.2320.fc5_tomoyo_1.6.0.spec || die "Can't rename spec file."
echo ""
echo ""
echo ""
echo "Edit /tmp/kernel-2.6.20-1.2320.fc5_tomoyo_1.6.0.spec if needed, and run"
echo "rpmbuild -bb /tmp/kernel-2.6.20-1.2320.fc5_tomoyo_1.6.0.spec"
