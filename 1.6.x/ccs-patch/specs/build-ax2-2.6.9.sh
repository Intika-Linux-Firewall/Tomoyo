...
if [ ! -r kernel-2.6.9-78.11AXS2.src.rpm ]
then
    wget http://ftp.miraclelinux.com/pub/Miracle/ia32/standard/4.0/updates/SRPMS/kernel-2.6.9-78.11AXS2.src.rpm || die "Can't download source package."
fi
rpm -ivh kernel-2.6.9-78.11AXS2.src.rpm || die "Can't install source package."
...
--- kernel-2.6.spec	2009-04-24 14:55:32.000000000 +0900
+++ kernel-2.6.spec	2009-04-30 17:58:14.000000000 +0900
@@ -34,7 +34,7 @@
 #
 %define axbsys %([ "%{?WITH_LKST}" -eq 0 ] && echo || echo .lkst)
 %define dist AXS2
-%define release 78.11%{?dist}%{axbsys}
+%define release 78.11%{?dist}%{axbsys}_tomoyo_1.6.7p1
 %define sublevel 9
