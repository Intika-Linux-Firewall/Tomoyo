...
apt-get source linux-source-3.0.0 || die "Can't install kernel source."
for i in `awk ' { if ( $1 != "Build-Depends:") next; $1 = ""; n = split($0, a, ","); for (i = 1; i <= n; i++) { split(a[i], b, " "); print b[1]; } } ' linux-3.0.0/debian/control`; do apt-get -y install $i; done

# Apply patches and create kernel config.
cd linux-3.0.0/ || die "Can't chdir to linux-3.0.0/ ."
update_maintainer
tar -zxf /root/rpmbuild/SOURCES/ccs-patch-1.8.3-20130117.tar.gz || die "Can't extract patch."
