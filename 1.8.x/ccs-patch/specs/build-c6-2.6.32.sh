...
if [ ! -r kernel-2.6.32-696.6.3.el6.src.rpm ]
then
    wget http://vault.centos.org/6.9/updates/Source/SPackages/kernel-2.6.32-696.6.3.el6.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-2.6.32-696.6.3.el6.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-2.6.32-696.6.3.el6.src.rpm || die "Can't install source package."
